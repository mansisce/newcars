...
/*code from header_jq_new.js start*/
...
//Poll if Js is loaded
var timeout = 100;
function isJsLoaded(file) {
	var deferred = new jQuery.Deferred();
	poll = function(file) {
		setTimeout(function () {
			var len = jQuery('script[src*="'+file+'"]').length;
		    timeout--;
		    if(len === 0) {
		    	if(timeout > 0) {
			    	poll(file);
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (result.Message == "Success") {
                ...
                //change modal message if mandatory verification for jobs enabled
                if(jQuery('#mandatory-mobile-verify-jobs').length) {
                	jQuery('#oneringapimsg').html('<span>Ads without verified mobile number will be deleted within 3 days of ad posting<br>To verify, Give a <b>Missed Call</b> to the below number<br></span><div><span class="mobtollfree">1800 200 4141</span></div><span>Its completely free!</span>');
                }
...
function setEGAcitySelect(obj){
	...
	if(gaurl != ''){
		...
		gen_comscore_tag("www."+testingUrl+".com/"+gaurl);
	}
...
function hidecityselect(format,from,uiver,noga){
	...
	if(noga!=1){
		...
		if(format == 'statewisecity'){
			...
			SetCookie('ipToCityStag',' ',testingUrl,'0');
...
var cookie = {
	...
	set: function(name, value, future, domain) {
		domain = (domain == undefined) ? '.'+testingUrl : domain;
		if(future == -1){
...
// function logInRegisterPopUp() { //new login function
...
// 	var el = jQuery("#signInRegisterHtml");	
// 	el.css("display","none");

	
//  	var el = jQuery("#signInSignUpHtml");	
// 	el.css("display","block");

// 	//jQuery("#checkpopup").val("sigin");
...
// 	jQuery('iframe').contents().find('._4z_b').css({'background-color':'none','background':'none'});
// 	jQuery('html,body').animate({scrollTop: jQuery("#signInSignUpHtml").offset().top},'slow');
...
var Recorder = function( config ){ 
  ...
  config.bitDepth =  32;
  config.bufferLength =  4096;
  config.enableMonitoring = false;
  config.numberOfChannels =  1;
  config.recordOpus =  true;
  config.sampleRate = 44100;
 
  config.workerPath = config.workerPath || '/getRemoteJs.php';
...
Recorder.prototype.getOgg = function( callback, mimeType ) {
  this.get( "ogg", function( data ){console.log('getogg');
...
function logInRegisterPopUp(strq) {
	

	var str_q = strq.value; 
	//alert(strq.value);
	//var el = jQuery("#newlogindiv").css("display","block");

	if($("#newlogindiv").is(':empty') ){

	jQuery.ajax({
			url : "/common?aj=1&for=registerpop&newLoginUI=true&rand="+Math.random(),
			type: 'POST',
			data : "source=register",
			dataType : "json"
		}).done(function(result){
			...
			jQuery('#newlogindiv').html(result.html);
			jQuery('#signin_error').hide();
	...
	var el = $("#signInSignUpHtml");
    el.css("display", "block");

	if(str_q!=''){
	jQuery("#checkpopup").val(str_q);
	}else{
	jQuery("#checkpopup").val(str_q);
	}	
	jQuery('html,body').animate({scrollTop: jQuery("#newlogindiv").offset().top},'slow');
...
function RegistersigninPopUp(strQ){
	var trans = jQuery('#trans');
	//Block Style
	trans.css("display","block");
	//trans.css("zIndex","10000");
	...
	el.css("display","block");
	if(strQ==''){
	jQuery("#checkpopup").val("signin");
	}else{
	jQuery("#checkpopup").val(strQ);
	}
	jQuery('html,body').animate({scrollTop: jQuery("#modal_container").offset().top},'slow');
...
function closeNewLoginPopup(){
	var elPopUp = jQuery("#signInSignUpHtml");
	elPopUp.fadeOut("slow");

	var elPopUp = jQuery("#signInRegisterHtml");
	elPopUp.fadeOut("slow");
...
function closeNewLoginPopupFinal(){
	var elPopUp = jQuery("#signInSignUpHtml");
	elPopUp.fadeOut("slow");
